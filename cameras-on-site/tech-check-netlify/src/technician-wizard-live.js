...
async function showITHome() {
  ...
  const [c,r,assignments,phoneAlerts] = await Promise.all([prepCounts(), returnCounts(), myActiveAssignments('it'), pushAlertState()]);
...
async function showSvcHome() {
  ...
  const [r, work, assignments, phoneAlerts] = await Promise.all([myReturnCounts(), serviceWorkData(), myActiveAssignments('service'), pushAlertState()]);
  const assigned = assignments[0] || null;
  const alertBanner = phoneAlertBanner(phoneAlerts);
