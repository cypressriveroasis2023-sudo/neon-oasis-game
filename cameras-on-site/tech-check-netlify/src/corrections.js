...
const EDIT_TYPES = ['Sniper', 'Ranger', 'Helios', 'Solar Spotter', 'Spotter', 'Recon 2', '110V Stand', 'Solar Stand'];
const EDIT_PURPOSES = ['BACKUP', 'SWAP', 'DELIVERY'];
const FIXED_BATTERIES = { Sniper: 2, Ranger: 1, Helios: 1, 'Solar Spotter': 4, Spotter: 0, '110V Stand': 0, 'Solar Stand': 0 };
...
function normalizedPurpose(type, selected) {
...
function purposeOptions(selected, type = 'Sniper') {
  const chosen = normalizedPurpose(type, selected);
  return allowedPurposes(type).map(p => '<option value="' + p + '"' + (p === chosen ? ' selected' : '') + '>' + p + '</option>').join('');
}
...
function structuralRow(item, prepId, temporary = false) {
  ...
  const purpose = normalizedPurpose(type, item.purpose || 'BACKUP');
  ...
  return '<div class="item cos-struct-row" data-prep-id="' + prepId + '" data-item-id="' + editEsc(id) + '">' +
    '<div class="grid3">' +
      ...
      '<div><label>Purpose</label><select class="cos-purpose">' + purposeOptions(purpose, type) + '</select></div>' +
...
async function renderITCorrections() {
  if (correctionRendering || !editRoleAllowed() || !editDb) return;
...
function wireCorrectionEvents(card) {
  ...
  card.addEventListener('change', e => {
    if (e.target.matches('.cos-eq')) {
      ...
      const purpose = row?.querySelector('.cos-purpose');
      if (purpose) purpose.innerHTML = purposeOptions(purpose.value, type);
    }
    if (e.target.matches('.cos-add-type')) {
      ...
      if (wrap) wrap.style.display = type === 'Recon 2' ? '' : 'none';
      const purpose = prep?.querySelector('.cos-add-purpose');
      if (purpose) purpose.innerHTML = purposeOptions(purpose.value, type);
    }
  });
  card.addEventListener('click', async e => {
    ...
    if (e.target.closest('.cos-add-row')) {
      ...
      if (!allowedPurposes(type).includes(purpose)) return alert('That stand / purpose combination is not available.');
...
async function saveStructure(prepEl, prepId) {
  ...
  for (const row of rows) {
    ...
    if (!allowedPurposes(type).includes(purpose)) return alert(type + ' is not available for ' + purpose + '.');
    items.push({
...
function restyleOwnerReset() {
  ...
  if (h2 && h2.textContent !== 'Reset All Workflow Data') h2.textContent = 'Reset All Workflow Data';
  const warning = card.querySelector('.warn');
  const warningHtml = '<b>Owner only — full operational reset.</b><div class="small">Clears ALL draft, released, and completed equipment prep; Service checkout verification history; Owner reports; and morning checks. Technician accounts, usernames, roles, and passwords are preserved.</div>';
  if (warning && warning.innerHTML !== warningHtml) warning.innerHTML = warningHtml;
  const button = card.querySelector('button');
  if (button && button.textContent !== 'Reset Everything — Keep Technician Accounts') button.textContent = 'Reset Everything — Keep Technician Accounts';
}

function installPrimaryStandOptions() {
  const typeSelect = document.getElementById('needType');
  const purposeSelect = document.getElementById('needPurpose');
  if (!typeSelect || !purposeSelect) return;
  for (const type of ['110V Stand', 'Solar Stand']) {
    if (![...typeSelect.options].some(o => o.value === type)) typeSelect.add(new Option(type, type));
  }
  const syncPurpose = () => {
    const current = purposeSelect.value;
    purposeSelect.innerHTML = purposeOptions(current, typeSelect.value);
  };
  if (typeSelect.dataset.standOptionsWired !== 'true') {
    typeSelect.dataset.standOptionsWired = 'true';
    typeSelect.addEventListener('change', syncPurpose);
  }
  syncPurpose();
}
...
function installCorrectionObservers() {
  ...
  if (owner) new MutationObserver(() => restyleOwnerReset()).observe(owner, { childList: true });
