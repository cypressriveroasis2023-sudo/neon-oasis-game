...
function legacyChecklist(item,unitNo,deps={}) {
  ...
  if (isHeliosDeploy(item)) {
    steps.push(
      ...
      { kind:'bool', field:'helios_camera1_ports_ok', label:`Camera 1 (.14): are device ports 81 / 554 open, and does the router forward 1400 / 1443 / 1454 to 192.168.10.14?` },
      { kind:'bool', field:'helios_camera2_ports_ok', label:`Camera 2 (.15): are device ports 81 / 554 open, and does the router forward 1500 / 1543 / 1554 to 192.168.10.15?` },
      { kind:'bool', field:'helios_ptz_ports_ok', label:`PTZ/180 (.16): are device ports 81 / 554 open, and does the router forward 1600 / 1643 / 1654 to 192.168.10.16?` },
      { kind:'bool', field:'helios_speaker_ports_ok', label:`IP Speaker (.17): are device ports 81 / 554 open, and does the router forward 1700 / 1743 / 1754 to 192.168.10.17?` },
