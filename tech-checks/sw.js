const CACHE_NAME = 'tech-check-field-shell-v57';
const APP_SHELL = './';
...
self.addEventListener('install', event => {
  event.waitUntil((async () => {
    ...
    await Promise.allSettled([
      cache.add(new Request('./app.js?v=startup-fast-v25', { cache:'reload' })),
      cache.add(new Request('./technician-wizard-owner-dashboard-v5.js?v=release-qa-v111', { cache:'reload' })),
      cache.add(new Request('./team-email-settings.js?v=email-settings-v4', { cache:'reload' })),
      cache.add(new Request('./styles.css?v=owner-assistant-v110', { cache:'reload' }))
    ]);
