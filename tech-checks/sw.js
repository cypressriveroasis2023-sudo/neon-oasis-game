const CACHE_NAME = 'tech-check-field-shell-v220-attention-contrast';
const APP_SHELL = './';
...
self.addEventListener('install', event => {
  event.waitUntil((async () => {
    ...
    await Promise.allSettled([
      ...
      cache.add(new Request('./app.js?v=owner-routing-fix-v218', { cache:'reload' })),
      cache.add(new Request('./technician-wizard-owner-dashboard-v5.js?v=owner-interactions-v207', { cache:'reload' })),
      cache.add(new Request('./team-email-settings.js?v=email-settings-v4', { cache:'reload' })),
      cache.add(new Request('./styles.css?v=owner-attention-contrast-v220', { cache:'reload' })),
